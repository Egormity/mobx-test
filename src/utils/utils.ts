...
export const subTaskAdding: SubTaskAddingProps = (id, array, task) => {
  return array.reduce((arr: TodoType[], item) => {
    if (item.id === id) {
        ...
        arr.push(item);
    } else arr.push({...item, subTasks: subTaskAdding(id, item.subTasks, task)});
    return arr;
  }, []);
};
...
export const recursionFilter: RecursionProps = (id, array) => {
  return array.reduce((arr: TodoType[], item) => {
    if (item.id !== id) arr.push({...item, subTasks: recursionFilter(id, item.subTasks)});
    return arr;
  }, []);
};
...
export const recursionCompleteToggler: RecursionProps = (id, array) => {
  return array.reduce((arr: TodoType[], item) => {
    if (item.id !== id) arr.push({...item, subTasks: recursionCompleteToggler(id, item.subTasks)});
    else {
        arr.push({
          ...item, 
